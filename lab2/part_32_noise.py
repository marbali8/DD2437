...
# out_val_ntr = delta_rbf(patt_tr, tar_tr_n, patt_val, tar_val, eta = 0.0001, epochs = 120, n_hidden = 20, width = 0.01)
# err_ntr = np.sqrt(np.mean((out_val_ntr - tar_val)**2))
# out_val_ntrte = delta_rbf(patt_tr, tar_tr_n, patt_val, tar_val_n, eta = 0.0001, epochs = 120, n_hidden = 20, width = 0.01)
# err_ntrte = np.sqrt(np.mean((out_val_ntrte - tar_val_n)**2))
#
# plt.figure(), plt.title('sin delta test outputs')
# plt.plot(patt_tr, tar_tr)
# plt.plot(patt_val, out_val)
# plt.plot(patt_val, out_val_ntr)
# plt.plot(patt_val, out_val_ntrte)
...
# err = np.sqrt(np.mean((out_val - tar_val)**2))
# out_val_ntr = batch_rbf(patt_tr, tar_tr_n, patt_val, tar_val, n_hidden = 20, width = 0.01)
# err_ntr = np.sqrt(np.mean((out_val_ntr - tar_val)**2))
# out_val_ntrte = batch_rbf(patt_tr, tar_tr_n, patt_val, tar_val_n, n_hidden = 20, width = 0.01)
# err_ntrte = np.sqrt(np.mean((out_val_ntrte - tar_val_n)**2))
#
